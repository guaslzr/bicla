<?php
include_once('global.php');
?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://fonts.googleapis.com/css?family=Sunflower:300" rel="stylesheet">
    <link rel="stylesheet" href="css/nosotros.css">
    <title>Nosotros</title>
  </head>
  <body>
    <header>
        <?php include_once('navigation.php') ?>
    </header>
      <h2>Nosotros somos BICLA</h2>
      <div class="info">
          <div class="contenedor">
             <img class="photo" src="img/team.jpg">
             <img class="logo" src="img/bicla_menu.png" width="100px;">
            <p>Bicla es una tienda de bicicletas que cuenta con una experiencia en el sector de más de 30 años donde la máxima de trabajo es cuidar la relación con nuestros clientes ofreciendo un servicio de calidad y confianza.
            Por eso trabajamos con las mejores marcas de bicicletas, manteniendo la máxima variedad de artículos en stock en nuestro catálogo para que los tengas todos con la máxima disponibilidad.
            <br><br>
            Bicla  nace a partir de la pasión despertada por el equipo Klever tras iniciarse en el mundo del ciclismo. Somos una empresa dedicada al desarrollo web, e-commerce y los servicios informáticos,
            y quisimos aprovechar estos conocimientos para unirlos con nuestra nueva pasión: el ciclismo. Sólo unas rutas por montaña bastaron para recuperar esa sensación increíble que deja el mundo de los pedales.</p>
          </div>
        </div>
    <footer>
  <?php include_once('footer.php'); ?>
    </footer>
      </body>
</html>
